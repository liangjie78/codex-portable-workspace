...
当本 Skill 在受限的 CC-Switch `--bare` worker 中调用时：

- 视 Codex 下发的任务为已经完成需求确认和风格选择，不重新向用户提问；缺少非阻塞信息时按任务说明中的默认值执行。
- 在额外目录中定位暂存的 `.claude/skills/guizang-ppt-skill/`，使用其中的 `assets/`、`references/` 和 `scripts/`。
- 安全模式下使用 Read/Write/Edit 创建目标目录内容和复制模板，不要照搬 `mkdir -p`、`cp` 等 Bash 写入命令。
- 不自行调用图片生成、浏览器或未授权外部服务；没有图片工具时使用任务允许的现有素材或明确占位槽。
- `validate-swiss-deck.mjs` 等最终校验由 Codex 作为 worker `checks` 显式配置，或由 Codex 在任务结束后亲自运行；不要为了运行校验扩大 Shell 权限。
- Codex 指定的允许路径、禁止路径、页数、风格、素材和验收标准优先于本 Skill 的通用示例。
