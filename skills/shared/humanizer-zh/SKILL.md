...
description: |
  ...
  当创建、修改或审阅中文用户可见文档时应自动考虑使用，尤其是 README.md、项目说明、
  使用指南、发布说明、方案文档、交付说明、博客草稿、营销文案和最终回复中的成段中文。
...
allowed-tools:
  ...
  - AskUserQuestion
metadata:
  trigger: 编辑或审阅中文文本，去除 AI 写作痕迹；处理 README、项目文档、说明文案、发布说明、博客草稿或最终交付文字时自动使用
...
在创建、编辑或审阅中文 README、项目说明、使用指南、发布说明、方案文档、交付说明、博客草稿、营销文案或其他用户可见长文本时，默认应用本技能做一次中文表达检查。目标是让文字更清楚、自然、具体，而不是把所有内容改成随意口语。

处理技术文档时，必须保留事实、参数、路径、命令、API 名称、配置键、版本号、表格数据和代码块。不要为了"人性化"改写可执行命令、文件路径、接口字段、错误信息、许可证文本或用户明确要求逐字保留的内容。
