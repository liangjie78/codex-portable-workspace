...
| `use_case` | 模型来源 | Effort | 异步默认超时 | 适合场景 |
| --- | --- | --- | --- | --- |
| `auto` | CC-Switch route | `max` | 无 | 普通实现 |
| `fast_patch` | CC-Switch route | `high` | 120s | 小补丁 |
| `simple_agent_task` | CC-Switch route | `high` | 180s | 简单 agentic coding |
| `scaffold_or_tests` | CC-Switch route | `high` | 300s | 脚手架、胶水代码、测试 |
| `debug_loop` | CC-Switch route | `max` | 无 | 复现、定位、修复、验证 |
| `agentic_coding` | CC-Switch route | `max` | 无 | 多步骤实现 |
| `complex_reasoning` | CC-Switch route | `max` | 无 | 架构和复杂逻辑 |
| `long_context_codebase` | CC-Switch route | `max` | 无 | 大上下文代码库 |
| `docs_generation` | CC-Switch route | `high` | 无 | 文档生成 |
