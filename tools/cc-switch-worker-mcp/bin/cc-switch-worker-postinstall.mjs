...
const setupScript = resolve(root, "bin/cc-switch-worker-setup.mjs");
const isGlobalInstall = process.env.npm_config_global === "true"
  || process.env.npm_config_location === "global";

if (process.env.CC_SWITCH_WORKER_SKIP_POSTINSTALL === "1") {
  process.exit(0);
}
...
process.stdout.write("\nCC-Switch Worker MCP postinstall\n\n");
...
function printNextSteps(reason) {
  ...
  process.stdout.write("  cc-switch-worker-setup\n");
  process.stdout.write("  cc-switch-worker-mcp --doctor\n\n");
  printMcpConfig();
}
